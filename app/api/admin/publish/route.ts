...
export async function POST(req: Request) {
  try {
    ...
    if (!finalSlug) {
      return NextResponse.json({ success: false, message: 'Could not generate a valid slug.' }, { status: 400 });
    }
    ...
    if (isDuplicate) {
      return NextResponse.json(
        { success: false, message: `A post with slug "${finalSlug}" already exists.` },
        { status: 409 }
      );
    }
